; DSH_PROVISIONING_BEGIN baseline-metadata
; DSH_TAURI_CLI_VERSION=2.11.4
; DSH_TAURI_BUNDLER_VERSION=2.9.4
; DSH_UPSTREAM_SHA256=20f4ecc730defb71f1342eaeaec4021df13be3d843abba0effe88ea5835fa079
; DSH_UPSTREAM_SOURCE=https://crates.io/crates/tauri-bundler/2.9.4
; Vendored from tauri-bundler 2.9.4. Keep the markers above in sync with
; scripts/verify-nsis-template.mjs before rebasing this custom template.
; DSH_PROVISIONING_END baseline-metadata
Unicode true
...
Function un.ConfirmShow ; Add add a `Delete app data` check box
  ...
  ; DSH_CUSTOM_BEGIN data-cleanup-checkbox
  ...
  IntOp $5 130 * $2
  IntOp $5 $5 / 96
  System::Call 'user32::CreateWindowEx(i r3, w "${__NSD_CheckBox_CLASS}", w "同时删除应用创建的本地项目（仅限文档 Projects 目录）", i ${__NSD_CheckBox_STYLE}, i r4, i r5, i r6, i r7, p $UninstallConfirmDialog, i0, i0, i0) i .s'
...
Section Uninstall
  ...
  ${AndIf} $UpdateMode <> 1
    DetailPrint "正在安全移出本地数据，磁盘空间将在后台释放..."
    ExecWait '"$INSTDIR\${MAINBINARYNAME}.exe" --cleanup-app-data' $0
    ${If} $0 <> 0
      DetailPrint "本地数据未完全删除（清理程序退出码：$0）"
      ${If} $PassiveMode <> 1
        MessageBox MB_ICONEXCLAMATION "应用将继续卸载，但本地数据未完全删除。请关闭占用文件的程序后重试。"
      ${EndIf}
    ${Else}
      ; Clear settings only after the data root was safely handed off.
