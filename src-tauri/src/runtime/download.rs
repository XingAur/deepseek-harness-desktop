    ...
    F: FnMut(u64, u64) + Send,
{
    if manifest.url.scheme() == "file" {
        let source = manifest.url.to_file_path().map_err(|_| {
            RuntimeFailure::new(RuntimeFailureCode::Network, "本地 Runtime URL 无效")
        })?;
    ...
    let response = request.send().await.map_err(|cause| {
        RuntimeFailure::new(
            ...
            format!("下载 Runtime 失败：{cause}"),
        )
    })?;
    if !response.status().is_success() {
        return Err(RuntimeFailure::new(
            RuntimeFailureCode::Network,
            format!("Runtime 下载返回 HTTP {}", response.status()),
        ));
    }
    ...
    // 每个 chunk 都有独立的停滞期限，连接半死时不会永远停在“下载中”。
    while let Some(chunk) = tokio::select! {
        _ = cancellation.cancelled() => return Err(RuntimeFailure::new(RuntimeFailureCode::Cancelled, "Runtime 下载已取消")),
        _ = tokio::time::sleep(Duration::from_secs(30)) => return Err(RuntimeFailure::new(RuntimeFailureCode::Network, "Runtime 下载连接停滞超时")),
    ...
    if completed != manifest.size {
        return Err(RuntimeFailure::new(
            RuntimeFailureCode::Network,
            format!("Runtime 下载不完整：{completed}/{}", manifest.size),
        ));
    }
