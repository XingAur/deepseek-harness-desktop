...
pub fn parse_and_verify_manifest(
    ...
    let manifest: RuntimeManifest = serde_json::from_value(value).map_err(|cause| {
        RuntimeFailure::new(
            RuntimeFailureCode::Signature,
            format!("运行时清单字段无效：{cause}"),
        )
    })?;
    ...
    fn sort(value: &Value, omitted_key: &str, root: bool) -> Value {
        match value {
            ...
            Value::Array(items) => Value::Array(
                items
                    .iter()
                    .map(|item| sort(item, omitted_key, false))
                    .collect(),
            ),
            scalar => scalar.clone(),
        }
...
mod tests {
    ...
    fn canonical_json_sorts_keys_and_omits_signature() {
        ...
        assert_eq!(
            canonical_payload(&input, "signature").unwrap(),
            br#"{"a":{"b":1,"c":2},"z":1}"#
        );
    }
