...
impl RuntimePaths {
    pub fn resolve(app: &AppHandle) -> Result<Self, RuntimeFailure> {
        let root = app.path().app_local_data_dir().map_err(RuntimeFailure::internal)?;
        let resource = app.path().resource_dir().map_err(RuntimeFailure::internal)?;
        let paths = Self {
    ...
    fn create(&self) -> Result<(), RuntimeFailure> {
        for path in [&self.versions, &self.downloads, &self.logs, &self.catalog, &self.diagnostics] {
...
pub fn validate_relative_path(value: &str, label: &str) -> Result<PathBuf, RuntimeFailure> {
    if value.is_empty() || value.contains('\0') {
        return Err(RuntimeFailure::new(RuntimeFailureCode::Archive, format!("{label} 为空或包含 NUL")));
    }
    let path = Path::new(value);
    if path.is_absolute() || path.components().any(|part| matches!(part, Component::ParentDir | Component::Prefix(_) | Component::RootDir)) {
        return Err(RuntimeFailure::new(RuntimeFailureCode::Archive, format!("{label} 必须位于运行时目录内")));
    }
    Ok(path.to_path_buf())
}
