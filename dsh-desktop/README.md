...
本项目使用 Tauri 2 + React + Rust 重写。目标不是给 DSH 再套一层管理后台，而是直接承载官方 DSH Web UI，并通过普通 DSH 插件组合桌面布局与社区插件市场。普通用户不需要预装 Node.js、pnpm 或 DSH。
...
- 成功启动后直接进入官方 DSH 工作台；桌面插件复用官方 sidebar、conversation、details 与 overlay slots。
...
签名校验 / 下载 / 解压 / 激活 / 回滚 / 诊断
       │
       ▼
Managed Node 24 + DSH 0.1.0-rc.7 + pnpm
       │ 127.0.0.1 随机端口
       ▼
官方 DSH Web UI + @dsh/desktop-plugin
官方工作台 slots + 社区插件市场
```

Tauri 页面只负责 Runtime 尚未就绪时的首启和恢复。进入 DSH 后，普通 Web 页面不持有通用 Tauri IPC，也不能任意执行本机命令。
...
- 校验目标平台与 DSH semver 范围；
- 只允许目录中固定的包名、版本和 GitHub HTTPS 仓库；
- 安装前明确提示第三方插件权限风险；
- 同一时间只允许一个插件写操作；
- 命令使用固定 Node/DSH 入口、参数数组和 `shell: false`；
...
- `packages/dsh-plugin-desktop/`：DSH Host/Client 插件、桌面布局和社区市场。
