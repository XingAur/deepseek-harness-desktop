...
run('tar', ['-xf', nodeArchive, '-C', downloads])
...
run('npm', ['install', '--omit=dev', '--ignore-scripts', '--no-audit', '--no-fund'], { cwd: appDir })
...
if (target === 'windows-x86_64') run('tar', ['-a', '-cf', archive, '.'], { cwd: stage })
else run('tar', ['-czf', archive, '.'], { cwd: stage })
...
function run(command, commandArgs, options = {}) {
  ...
  const result = spawnSync(executable, commandArgs, { cwd: options.cwd ?? process.cwd(), stdio: 'inherit', shell: false })
