#!/usr/bin/env python3
"""Strict L4 gate for one immutable-plan GitHub delivery action."""
...
def _result(request_id: str, status: str, summary: str, *, data: Mapping[str, Any], blockers: list[str]) -> dict[str, Any]:
    return {
        ...
        "audit": {"credential_class": "github_write", "external_write_attempted": False},
    }
...
def _validate(payload: object) -> tuple[dict[str, Any] | None, str]:
    ...
    if (
        ...
        or payload.get("capability") != "github.write"
        or payload.get("provider") != "his-engineering"
        or payload.get("mode") != "apply"
        or payload.get("mutation_level") != "L4"
        or payload.get("authorization") != {"explicit": True, "scope": ["github:write", "capability:github.write"]}
...
def execute_request(payload: object) -> dict[str, Any]:
    ...
    if request is None:
        return _result(request_id, "blocked", "GITHUB_WRITE_BLOCKED", data={}, blockers=[error])
    data = request["input"]
    database_path = Path(data["delivery_db"])
    if not database_path.is_file() or database_path.is_symlink():
        return _result(request["request_id"], "blocked", "GITHUB_WRITE_BLOCKED", data={}, blockers=["delivery_store_unavailable"])
    try:
        ...
        if (
            transaction.get("state") != "github_delivery_pending"
            ...
            return _result(request["request_id"], "blocked", "GITHUB_WRITE_BLOCKED", data={}, blockers=["github_delivery_not_ready"])
    except DeliveryError as exc:
        return _result(request["request_id"], "blocked", "GITHUB_WRITE_BLOCKED", data={"error_code": exc.code}, blockers=[exc.code])
    except Exception:
        return _result(request["request_id"], "blocked", "GITHUB_WRITE_BLOCKED", data={}, blockers=["delivery_runtime_unavailable"])
    return _result(request["request_id"], "success", "GITHUB_WRITE_READY", data={"github_action": dict(action)}, blockers=[])
