...
const FULL_UPGRADE_SPEC = 'e2e/specs/provisioning-upgrade.installer.e2e.ts'
...
export function assertInstallerSuiteReady(mode, options = {}) {
  ...
  const artifactsRoot = resolve(cwd, env.DSH_E2E_ARTIFACTS ?? 'e2e-artifacts')
  let metadata
  try {
  ...
  if (mode === 'full' && !exists(resolve(cwd, FULL_UPGRADE_SPEC))) {
    throw new Error('full 安装 E2E 尚未接入升级验证 spec，已拒绝执行')
  }
}

export function runInstallerSuite(mode, { cwd = process.cwd(), spawnProcess = spawn, env = process.env, readFile = readFileSync, exists = existsSync } = {}) {
  const executionEnv = { ...env, DSH_E2E_MODE: mode }
  assertInstallerSuiteReady(mode, { cwd, env: executionEnv, readFile, exists })
