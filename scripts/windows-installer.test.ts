...
describe('Windows installer contract', () => {
  ...
  it('verifies the signed Windows Runtime and rejects a changed archive', () => {
    ...
    try {
      ...
      const manifest = {
        ...
        url: 'https://github.com/example/repo/releases/download/runtime-v0.1.8-preview/dsh-runtime-windows-x86_64.zip',
        size: archive.length,
      ...
      expect(() => verifyBundledRuntime({
        ...
        archivePath,
        publicKey: publicJwk.x!,
      })).toThrow(/runtime-v0.1.8-preview/)
      writeFileSync(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`)
      expect(() => verifyBundledRuntime({
        manifestPath,
  ...
  it('pins the managed Runtime release version', () => {
    expect(MANAGED_RUNTIME_VERSION).toBe('0.1.8-preview')
  })
      ...
      const generatedConfig = prepareWindowsInstallerConfig({
        ...
        environment: {
          ...
          DSH_DESKTOP_RUNTIME_MANIFEST_URL:
            'https://github.com/example/repo/releases/download/runtime-v0.1.8-preview/runtime-{target}.json',
        },
      })
