...
export function installAdvancedStyles(): () => void {
  ...
  style.textContent = `
    ...
    body[data-dsh-desktop-mode="advanced"] { color: var(--dsw-alias-label-primary, #202126); --dsh-desktop-divider: var(--dsw-alias-border-secondary, rgba(127,127,127,.16)); }
    body[data-dsh-desktop-mode="advanced"][data-dsh-desktop-theme="light"] { --dsh-desktop-divider: rgba(29,38,58,.035); }
    .dshDesktopFrame { position: relative; display: grid; width: 100%; height: 100%; min-width: 0; background: var(--dsw-alias-bg-base, #ffffff); }
    ...
    .dshDesktopSidebarSurface { position: relative; display: flex; flex-direction: column; background: var(--dsw-specific-sidebar-fill, var(--dsw-alias-bg-layer-2, #f0f2f7)); border-right: 1px solid var(--dsh-desktop-divider); }
    .dshDesktopUpstreamSidebar { flex: 1; min-height: 0; }
    .dshDesktopFooterAction { box-sizing: border-box; display: flex; align-items: center; width: calc(100% + 4px); height: 42px; margin: 4px -2px; padding: 0 10px 0 8px; gap: 8px; border: 0; border-radius: 12px; color: var(--dsw-alias-label-primary, #202126); background: transparent; font: 400 14px/22px var(--dsw-font-family, system-ui, sans-serif); text-align: left; cursor: pointer; overflow: hidden; transition: background-color .15s ease; }
    ...
    .dshDesktopConversationSurface { background: var(--dsw-alias-bg-base, #ffffff); }
    .dshDesktopDetailsSurface { background: var(--dsw-alias-bg-layer-1, #ffffff); border-left: 1px solid var(--dsh-desktop-divider); }
    ...
    .dshDesktopProjectsPage { box-sizing: border-box; height: 100%; overflow: auto; padding: 30px 38px 0; color: var(--dsw-alias-label-primary, #202126); background: var(--dsw-alias-bg-base, #ffffff); }
    .dshDesktopProjectsPageInner { width: min(1180px, 100%); min-height: 100%; margin: 0 auto; }
    .dshDesktopProfileSettings { display: grid; gap: 18px; color: var(--dsw-alias-label-primary, #202126); }
    ...
    .dshDesktopProfileSettings header p { margin: 6px 0 0; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 13px; }
    .dshDesktopProfileSettings button, .dshDesktopProfileEditor input, .dshDesktopProfileEditor select { min-height: 34px; border: 1px solid var(--dsh-desktop-divider); border-radius: 8px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); }
    ...
    .dshDesktopProfileEditor { display: grid; grid-template-columns: 1fr 1.5fr 150px auto; align-items: end; gap: 10px; padding: 14px; border: 1px solid var(--dsh-desktop-divider); border-radius: 12px; background: var(--dsw-alias-bg-layer-1, #ffffff); }
    .dshDesktopProfileEditor label { display: grid; gap: 5px; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12px; }
    ...
    .dshDesktopProfileSettingsList p { overflow: hidden; margin: 0; color: var(--dsw-alias-label-secondary, #687081); text-overflow: ellipsis; white-space: nowrap; }
    .dshDesktopProfileSettingsList small { grid-column: 1 / 3; color: var(--dsw-alias-label-tertiary, #8a92a1); }
    .dshDesktopProfileSettingsActions { grid-column: 3; grid-row: 1 / 3; display: flex; gap: 6px; }
    .dshModelAgentCenter { box-sizing: border-box; display: grid; gap: 18px; min-height: 100%; padding: 28px 34px 44px; color: var(--dsw-alias-label-primary, #202126); background: var(--dsw-alias-bg-base, #ffffff); }
    ...
    .dshModelAgentCenterHeader > div > p:last-child { margin: 7px 0 0; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 13px; }
    .dshModelAgentCenterHeader > button, .dshModelAgentActions button, .dshModelAgentDialog footer button { min-height: 34px; padding: 0 12px; border: 1px solid var(--dsh-desktop-divider); border-radius: 8px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); cursor: pointer; }
    .dshModelAgentCenter button:disabled { cursor: default; opacity: .48; }
    .dshModelAgentHarnessEntry { display: flex; align-items: center; justify-content: space-between; gap: 14px; padding: 13px 15px; border: 1px solid var(--dsh-desktop-divider); border-radius: 12px; background: var(--dsw-alias-bg-layer-1, #ffffff); }
    .dshModelAgentHarnessEntry > div { display: grid; gap: 3px; min-width: 0; }
    .dshModelAgentHarnessEntry strong { font-size: 13px; }
    .dshModelAgentHarnessEntry span { color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12px; }
    .dshModelAgentHarnessEntryToggle { position: relative; width: 40px; height: 22px; flex: 0 0 auto; padding: 0; border: 0; border-radius: 999px; background: var(--dsw-alias-bg-layer-2, #f0f2f7); cursor: pointer; transition: background-color .15s ease; }
    .dshModelAgentHarnessEntryToggle::after { content: ""; position: absolute; top: 3px; left: 3px; width: 16px; height: 16px; border-radius: 50%; background: var(--dsw-alias-label-tertiary, #8a92a1); transition: transform .15s ease; }
    ...
    .dshModelAgentTabs button { min-height: 32px; padding: 0 12px; border: 0; border-radius: 8px; color: var(--dsw-alias-label-tertiary, #8a92a1); background: transparent; cursor: pointer; }
    .dshModelAgentTabs button[aria-selected="true"] { color: var(--dsw-alias-label-primary, #202126); background: color-mix(in srgb, #7196ff 15%, transparent); }
    .dshModelAgentGrid { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 12px; align-content: start; }
    .dshModelAgentCard { display: grid; gap: 12px; padding: 16px; border: 1px solid var(--dsh-desktop-divider); border-radius: 14px; background: var(--dsw-alias-bg-layer-1, #ffffff); }
    ...
    .dshModelAgentCardHeader small { display: block; margin-top: 3px; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11px; }
    ...
    .dshModelAgentCardText, .dshModelAgentCardHint { margin: 0; color: var(--dsw-alias-label-secondary, #687081); font-size: 12px; line-height: 1.55; }
    .dshModelAgentCardHint { color: #e8aaaa; }
    .dshModelAgentCardMeta { display: flex; flex-wrap: wrap; gap: 7px 14px; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11px; }
    ...
    .dshModelAgentDetails dt { color: var(--dsw-alias-label-tertiary, #8a92a1); }
    .dshModelAgentDetails dd { overflow: hidden; margin: 0; text-overflow: ellipsis; white-space: nowrap; }
    .dshModelAgentEmpty, .dshModelAgentMuted { color: var(--dsw-alias-label-tertiary, #8a92a1); }
    ...
    .dshModelAgentSummary { display: flex; align-items: baseline; gap: 7px; padding: 14px 16px; border: 1px solid var(--dsh-desktop-divider); border-radius: 12px; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12px; }
    .dshModelAgentSummary strong { margin-left: 12px; color: var(--dsw-alias-label-primary, #202126); font-size: 20px; }
    ...
    .dshModelAgentDiagnosticRow { display: flex; justify-content: space-between; gap: 12px; padding: 9px 11px; border-radius: 8px; background: var(--dsw-alias-bg-layer-1, #ffffff); font-size: 12px; }
    .dshModelAgentDiagnosticRow > span:last-child { color: var(--dsw-alias-label-tertiary, #8a92a1); }
    ...
    .dshModelAgentDialog { display: grid; gap: 14px; width: min(430px, 100%); box-sizing: border-box; padding: 20px; border: 1px solid color-mix(in srgb, var(--dsh-desktop-divider) 75%, #8c99b8 25%); border-radius: 16px; background: var(--dsw-alias-bg-layer-1, #ffffff); box-shadow: 0 28px 80px rgba(0,0,0,.38); }
    ...
    .dshModelAgentDialog header button { border: 0; color: var(--dsw-alias-label-tertiary, #8a92a1); background: transparent; font-size: 24px; cursor: pointer; }
    .dshModelAgentDialogHint { margin: 0; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12px; line-height: 1.55; }
    .dshModelAgentField { display: grid; gap: 6px; color: var(--dsw-alias-label-secondary, #687081); font-size: 12px; }
    .dshModelAgentField input { box-sizing: border-box; width: 100%; min-height: 39px; padding: 0 10px; border: 1px solid var(--dsh-desktop-divider); border-radius: 8px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); }
    .dshModelAgentField textarea { box-sizing: border-box; width: 100%; padding: 8px 10px; border: 1px solid var(--dsh-desktop-divider); border-radius: 8px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); font: inherit; resize: vertical; }
    ...
    .dshModelAgentSecondary { min-height: 39px; align-self: end; padding: 0 12px; border: 1px solid var(--dsh-desktop-divider); border-radius: 8px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); cursor: pointer; white-space: nowrap; }
    ...
    .dshModelAgentAdvanced { display: grid; gap: 10px; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12px; }
    ...
    .dshExtensionReviewFacts dt { color: var(--dsw-alias-label-tertiary, #8a92a1); }
    ...
    .dshMcpServerSection li { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 7px 9px; border-radius: 8px; background: var(--dsw-alias-bg-layer-2, #f0f2f7); font-size: 12px; }
    .dshMcpServerSection li span { color: var(--dsw-alias-label-tertiary, #8a92a1); }
    ...
    .dshDesktopProjectCard { position: relative; border: 1px solid var(--dsh-desktop-divider); border-radius: 15px; background: var(--dsw-alias-bg-layer-1, #ffffff); animation: dshDesktopProjectEnter 180ms ease-out both; transition: transform 160ms ease, border-color 160ms ease, box-shadow 160ms ease; }
    ...
    .dshDesktopProjectCardBody > p { overflow: hidden; margin: 0; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12px; text-overflow: ellipsis; white-space: nowrap; }
    .dshDesktopProjectCardBody > input { box-sizing: border-box; width: 100%; min-height: 29px; margin: -5px 0 3px; padding: 0 7px; border: 1px solid #7196ff; border-radius: 6px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); }
    .dshDesktopProjectMeta { display: flex; justify-content: space-between; gap: 10px; margin: 15px 0 0; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11px; }
    ...
    .dshDesktopProjectContextMenu { position: fixed; z-index: 80; display: grid; width: 188px; padding: 6px; border: 1px solid color-mix(in srgb, var(--dsh-desktop-divider) 72%, #7196ff 28%); border-radius: 11px; color: var(--dsw-alias-label-primary, #202126); background: color-mix(in srgb, var(--dsw-alias-bg-layer-1, #ffffff) 96%, transparent); box-shadow: 0 18px 45px rgba(0,0,0,.32); backdrop-filter: blur(18px); }
    ...
    .dshDesktopProjectDeleteDialog { box-sizing: border-box; width: min(470px, 100%); padding: 20px; border: 1px solid color-mix(in srgb, var(--dsh-desktop-divider) 75%, #8c99b8 25%); border-radius: 16px; color: var(--dsw-alias-label-primary, #202126); background: var(--dsw-alias-bg-layer-1, #ffffff); box-shadow: 0 28px 80px rgba(0,0,0,.38); }
    ...
    .dshDesktopProjectDeleteDialog > header button { border: 0; color: var(--dsw-alias-label-tertiary, #8a92a1); background: transparent; font-size: 24px; cursor: pointer; }
    .dshDesktopProjectDeletePath { overflow: hidden; margin: 12px 0 16px; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12px; text-overflow: ellipsis; white-space: nowrap; }
    ...
    .dshDesktopProjectDeleteDialog fieldset small { color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11px; }
    .dshDesktopProjectDeleteNameCheck { display: grid; gap: 6px; margin-top: 14px; color: var(--dsw-alias-label-secondary, #687081); font-size: 12px; }
    .dshDesktopProjectDeleteNameCheck input { box-sizing: border-box; width: 100%; min-height: 38px; padding: 0 10px; border: 1px solid var(--dsh-desktop-divider); border-radius: 8px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); }
    ...
    .dshDesktopProjectDeleteDialog > footer button { min-height: 37px; padding: 0 14px; border: 1px solid var(--dsh-desktop-divider); border-radius: 8px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); cursor: pointer; }
    ...
    .dshDesktopProjectSkeletons span { height: 160px; border-radius: 15px; background: var(--dsw-alias-bg-layer-1, #ffffff); opacity: .65; }
    ...
    .dshDesktopProjectEmpty p { margin: 0; color: var(--dsw-alias-label-tertiary, #8a92a1); }
    .dshDesktopProjectComposerDock { position: sticky; bottom: 0; z-index: 4; margin-top: 24px; padding: 12px 0 22px; background: linear-gradient(180deg, transparent, var(--dsw-alias-bg-base, #ffffff) 18%); }
    ...
    .dshDesktopAdoptList small { overflow: hidden; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }
    .dshDesktopProjectComposer, .dshDesktopProjectConfirm { display: grid; gap: 10px; width: min(840px, 100%); box-sizing: border-box; margin: 0 auto; padding: 12px 16px 13px; border: 1px solid var(--dsh-desktop-divider); border-radius: 16px; text-align: left; background: var(--dsw-alias-bg-layer-1, #ffffff); box-shadow: 0 14px 36px color-mix(in srgb, #0b1020 14%, transparent); transition: border-color .15s ease, box-shadow .15s ease; }
    ...
    .dshDesktopProjectComposerBar { display: flex; align-items: center; justify-content: space-between; gap: 12px; min-height: 26px; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12px; }
    ...
    .dshDesktopProjectComposerContext strong { overflow: hidden; color: var(--dsw-alias-label-primary, #202126); font-size: 13px; font-weight: 600; text-overflow: ellipsis; white-space: nowrap; }
    ...
    .dshDesktopProjectComposerClear { flex: 0 0 auto; width: 26px; height: 26px; display: grid; place-items: center; border: 0; border-radius: 50%; color: var(--dsw-alias-label-tertiary, #8a92a1); background: transparent; font-size: 19px; line-height: 1; cursor: pointer; }
    .dshDesktopProjectComposerClear:hover:not(:disabled) { color: var(--dsw-alias-label-primary, #202126); background: var(--dsw-alias-interactive-bg-hover, rgba(127,127,127,.1)); }
    .dshDesktopProjectComposerClear:disabled { cursor: default; opacity: .48; }
    .dshDesktopProjectComposer > textarea { box-sizing: border-box; width: 100%; min-height: 88px; padding: 2px 2px 4px; border: 0; resize: vertical; color: var(--dsw-alias-label-primary, #202126); background: transparent; font: 400 14px/22px var(--dsw-font-family, system-ui, sans-serif); outline: none; }
    .dshDesktopProjectComposer > textarea::placeholder { color: var(--dsw-alias-label-tertiary, #8a92a1); }
    .dshDesktopProjectComposer > textarea:disabled { opacity: .55; }
    .dshDesktopProjectComposerFooter { min-height: 34px; }
    .dshDesktopProjectComposerHint { overflow: hidden; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }
    ...
    .dshDesktopProjectComposerActions button { min-height: 36px; padding: 0 14px; border: 1px solid var(--dsh-desktop-divider); border-radius: 999px; color: var(--dsw-alias-label-primary, #202126); background: var(--dsw-alias-bg-layer-2, #f0f2f7); cursor: pointer; }
    ...
    .dshDesktopProjectConfirm dt { color: var(--dsw-alias-label-tertiary, #8a92a1); }
    ...
    .dshAgentWorkbench { display: grid; gap: 16px; margin-top: 22px; padding: 18px; border: 1px solid var(--dsh-desktop-divider); border-radius: 16px; background: var(--dsw-alias-bg-layer-1, #ffffff); }
    ...
    .dshAgentWorkbenchHeader p:last-child { margin-top: 4px; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12px; }
    .dshAgentWorkbenchHeader button, .dshAgentWorkbenchActions button, .dshAgentWorkbenchApproval button, .dshAgentWorkbenchTimeline button { min-height: 30px; padding: 0 10px; border: 1px solid var(--dsh-desktop-divider); border-radius: 8px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); cursor: pointer; }
    ...
    .dshAgentWorkbenchCreate label { display: grid; gap: 5px; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11px; }
    .dshAgentWorkbenchCreate select, .dshAgentWorkbenchCreate textarea { box-sizing: border-box; min-height: 34px; padding: 7px 9px; border: 1px solid var(--dsh-desktop-divider); border-radius: 8px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); font: inherit; }
    ...
    .dshAgentWorkbenchTask span, .dshAgentWorkbenchTask small, .dshAgentWorkbenchApproval span { color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11px; }
    ...
    .dshAgentWorkbenchTimeline li { padding-left: 3px; color: var(--dsw-alias-label-secondary, #687081); font-size: 12px; }
    ...
    .dshAgentWorkbenchDiff { padding: 12px; border: 1px solid var(--dsh-desktop-divider); border-radius: 10px; background: var(--dsw-alias-bg-layer-2, #f0f2f7); }
    ...
    .dshModelAgentWorkbenchHint { margin: 4px 0 0; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12.5px; line-height: 1.7; }
    .dshModelAgentWorkbenchHint strong { color: var(--dsw-alias-label-secondary, #687081); }
    ...
    .dshExtCenterHeader p { margin: 5px 0 0; max-width: 62ch; color: var(--dsw-alias-label-secondary, #687081); font-size: 13px; line-height: 1.65; }
    ...
    .dshExtCenterTabs button { min-height: 28px; padding: 4px 12px; border: 0; background: transparent; color: var(--dsw-alias-label-secondary, #687081); cursor: pointer; }
    .dshExtCenterTabs button[aria-selected='true'] { color: var(--dsw-alias-label-primary, #202126); border-color: var(--dsw-alias-border-secondary, currentColor); }
    .dshExtCenterPlaceholder { color: var(--dsw-alias-label-secondary, #687081); padding: 32px 0; text-align: center; }
    .dshExtErrorCard { display: grid; gap: 10px; justify-items: start; border: 1px solid #b45309; border-radius: 12px; padding: 18px 20px; background: color-mix(in srgb, #b45309 8%, transparent); }
    .dshExtErrorCard h3 { margin: 0; font-size: 15px; }
    .dshExtErrorCard p { margin: 0; color: var(--dsw-alias-label-secondary, #687081); font-size: 13px; }
    ...
    .dshPluginsPanelHead p { margin: 0; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12px; }
    ...
    .dshPluginMarketHead p { margin: 0; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12px; }
    ...
    .dshPluginMarketControls input { box-sizing: border-box; width: 100%; max-width: 420px; min-height: 34px; padding: 0 12px; border: 1px solid var(--dsh-desktop-divider); border-radius: 10px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); font: 13px inherit; }
    ...
    .dshPluginMarketCategories button { min-height: 26px; padding: 0 10px; border: 1px solid var(--dsh-desktop-divider); border-radius: 999px; color: var(--dsw-alias-label-secondary, #687081); background: transparent; font-size: 11.5px; cursor: pointer; }
    ...
    .dshPluginCard { display: grid; grid-template-columns: 1fr auto; gap: 12px; align-items: center; padding: 15px 16px; border: 1px solid var(--dsh-desktop-divider); border-radius: 14px; background: var(--dsw-alias-bg-layer-1, #ffffff); }
    .dshPluginCard.is-featured { grid-column: 1 / -1; border-color: color-mix(in srgb, #9db2ff 45%, var(--dsh-desktop-divider)); background: linear-gradient(120deg, color-mix(in srgb, #5877cf 12%, var(--dsw-alias-bg-layer-1, #ffffff)), var(--dsw-alias-bg-layer-1, #ffffff)); }
    ...
    .dshPluginCardTitle small { color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11px; overflow-wrap: anywhere; }
    .dshPluginCardTitle a { color: #9db2ff; text-decoration: none; }
    .dshPluginCardTitle a:hover { text-decoration: underline; }
    .dshPluginCard p { margin: 0; color: var(--dsw-alias-label-secondary, #687081); font-size: 12px; line-height: 1.55; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; }
    ...
    .dshPluginJobLog { grid-column: 1 / -1; max-height: 120px; margin: 0; padding: 9px 11px; border: 1px solid var(--dsh-desktop-divider); border-radius: 9px; overflow: auto; color: var(--dsw-alias-label-secondary, #687081); background: var(--dsw-alias-bg-base, #ffffff); font: 11px/1.6 ui-monospace, SFMono-Regular, monospace; white-space: pre-wrap; overflow-wrap: anywhere; }
    ...
    .dshAgentHomeLead { margin: 6px 0 0; max-width: 56ch; color: var(--dsw-alias-label-secondary, #687081); font-size: 13px; line-height: 1.65; }
    .dshAgentCard { display: grid; gap: 16px; margin-top: 18px; padding: 20px 22px; border: 1px solid var(--dsh-desktop-divider); border-radius: 16px; background: var(--dsw-alias-bg-layer-1, #ffffff); }
    ...
    .dshAgentPill.is-muted { color: var(--dsw-alias-label-tertiary, #8a92a1); background: color-mix(in srgb, #85858d 12%, transparent); }
    ...
    .dshAgentStepMark { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center; width: 22px; height: 22px; margin-top: 1px; border-radius: 50%; border: 1.5px solid var(--dsh-desktop-divider); background: var(--dsw-alias-bg-layer-1, #ffffff); color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11.5px; font-weight: 600; }
    .dshAgentStep[data-state="done"] .dshAgentStepMark { border-color: color-mix(in srgb, #6faa7d 70%, transparent); color: #8fd3a0; background: color-mix(in srgb, #6faa7d 16%, var(--dsw-alias-bg-layer-1, #ffffff)); }
    .dshAgentStep[data-state="active"] .dshAgentStepMark { border-color: color-mix(in srgb, #9db2ff 75%, transparent); color: #cfd9ff; background: color-mix(in srgb, #5877cf 22%, var(--dsw-alias-bg-layer-1, #ffffff)); }
    ...
    .dshAgentStep[data-state="done"] .dshAgentStepCopy strong { color: var(--dsw-alias-label-secondary, #687081); }
    .dshAgentStepCopy p { margin: 3px 0 0; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12px; line-height: 1.6; overflow-wrap: anywhere; }
    .dshAgentReadyNote { margin: 0; color: var(--dsw-alias-label-secondary, #687081); font-size: 12.5px; line-height: 1.7; }
    ...
    .dshAgentGhostButton { min-height: 34px; padding: 0 13px; border: 1px solid var(--dsh-desktop-divider); border-radius: 10px; color: var(--dsw-alias-label-primary, #202126); background: transparent; font-size: 12.5px; cursor: pointer; transition: background-color .15s ease; }
    ...
    .dshAgentComingHint { margin: 0; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12.5px; line-height: 1.6; }
    .dshAgentLog { border: 1px solid var(--dsh-desktop-divider); border-radius: 12px; overflow: hidden; background: var(--dsw-alias-bg-base, #ffffff); }
    .dshAgentLog summary { padding: 8px 12px; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11.5px; cursor: pointer; user-select: none; }
    .dshAgentLogHead { display: flex; align-items: center; justify-content: space-between; padding: 8px 12px; border-bottom: 1px solid var(--dsh-desktop-divider); color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11px; letter-spacing: .4px; }
    .dshAgentLogLive { color: #9db2ff; }
    .dshAgentLog pre { max-height: 168px; margin: 0; padding: 11px 13px; overflow: auto; color: var(--dsw-alias-label-secondary, #687081); font: 11.5px/1.65 ui-monospace, SFMono-Regular, monospace; white-space: pre-wrap; overflow-wrap: anywhere; }
    .dshAgentAdvanced { border-top: 1px dashed var(--dsh-desktop-divider); padding-top: 4px; }
    .dshAgentAdvanced summary { padding: 8px 0; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12px; cursor: pointer; user-select: none; }
    .dshAgentAdvanced summary:hover { color: var(--dsw-alias-label-secondary, #687081); }
    .dshAgentAdvancedBody { display: grid; gap: 9px; }
    .dshAgentAdvancedBody > p { margin: 0; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12px; line-height: 1.6; }
    .dshAgentAdvancedBody code { padding: 1px 5px; border-radius: 5px; background: var(--dsw-alias-bg-layer-2, #f0f2f7); font: 11px ui-monospace, SFMono-Regular, monospace; }
    .dshAgentAdvancedRow { display: grid; grid-template-columns: 1fr auto; gap: 8px; }
    .dshAgentAdvancedRow input { box-sizing: border-box; min-height: 34px; padding: 0 11px; border: 1px solid var(--dsh-desktop-divider); border-radius: 10px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); font: 12px ui-monospace, SFMono-Regular, monospace; }
    .dshAgentAdvancedRow input:focus-visible { outline: 2px solid var(--dsw-alias-state-business-primary, #7d9cf0); outline-offset: 1px; }
    .dshAgentAdvancedResult { margin: 0; color: var(--dsw-alias-label-secondary, #687081); font-size: 12px; line-height: 1.6; overflow-wrap: anywhere; }
    ...
    .dshAgentWorkbenchHostHeader button { min-height: 30px; padding: 0 10px; border: 1px solid var(--dsh-desktop-divider); border-radius: 8px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); cursor: pointer; }
    ...
    .dshPromptsTargetState { color: var(--dsw-alias-label-secondary, #687081); font-size: 12px; }
    ...
    .dshPromptsMuted { color: var(--dsw-alias-label-secondary, #687081); font-size: 12px; }
    .dshPromptsDialogBackdrop { position: fixed; inset: 0; background: rgba(0,0,0,.45); display: grid; place-items: center; z-index: 40; }
    .dshPromptsDialog { background: var(--dsw-alias-bg-layer-1, #ffffff); color: inherit; border-radius: 14px; padding: 18px 20px; width: min(480px, 90vw); display: grid; gap: 12px; }
    ...
    .dshPromptsModeSwitch button[aria-pressed='true'] { background: var(--dsw-alias-interactive-bg-hover, rgba(127,127,127,.16)); border-color: var(--dsw-alias-border-secondary, rgba(127,127,127,.6)); }
    ...
    .dshSkills { display: grid; gap: 14px; }
    .dshSkillsToolbar { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
    .dshSkillsToolbar button { border: 1px solid var(--dsw-alias-border-secondary, rgba(127,127,127,.4)); border-radius: 8px; padding: 5px 12px; background: transparent; color: inherit; cursor: pointer; }
    .dshSkillsToolbar button:hover:not(:disabled) { background: var(--dsw-alias-interactive-bg-hover, rgba(127,127,127,.1)); }
    .dshSkillsToolbar button:disabled { opacity: .45; cursor: not-allowed; }
    .dshSkillsSpacer { flex: 1; }
    .dshSkillsGroup { display: grid; gap: 8px; align-content: start; }
    .dshSkillsGroupHeader { display: flex; align-items: center; gap: 10px; }
    .dshSkillsGroupHeader > span:first-child { font-weight: 600; }
    .dshSkillsTargetState { color: var(--dsw-alias-label-secondary, #687081); font-size: 12px; }
    .dshSkillsTargetState[data-installed="false"] { opacity: .7; }
    .dshSkillsTargetPath { margin-left: auto; color: var(--dsw-alias-label-tertiary, #8a92a1); font: 12px/1.6 ui-monospace, monospace; overflow-wrap: anywhere; }
    .dshSkillsList { list-style: none; margin: 0; padding: 0; display: grid; gap: 6px; align-content: start; }
    .dshSkillsListItem { display: flex; align-items: center; gap: 10px; border: 1px solid var(--dsw-alias-border-secondary, rgba(127,127,127,.25)); border-radius: 10px; padding: 8px 10px; }
    .dshSkillsItemMain { display: grid; gap: 2px; min-width: 0; flex: 1; }
    .dshSkillsMuted { margin: 0; color: var(--dsw-alias-label-secondary, #687081); font-size: 12px; overflow-wrap: anywhere; }
    .dshSkillsItemActions { display: flex; gap: 8px; flex: 0 0 auto; }
    .dshSkillsItemActions button { border: 1px solid var(--dsw-alias-border-secondary, rgba(127,127,127,.4)); border-radius: 8px; padding: 4px 10px; background: transparent; color: inherit; cursor: pointer; }
    .dshSkillsItemActions button:disabled { opacity: .45; cursor: not-allowed; }
    .dshSkillsDialogBackdrop { position: fixed; inset: 0; background: rgba(0,0,0,.45); display: grid; place-items: center; z-index: 40; }
    .dshSkillsDialog { background: var(--dsw-alias-bg-layer-1, #ffffff); color: inherit; border-radius: 14px; padding: 18px 20px; width: min(520px, 90vw); display: grid; gap: 12px; }
    .dshSkillsDialog h3 { margin: 0; }
    .dshSkillsField { display: grid; gap: 4px; }
    .dshSkillsField > span { color: var(--dsw-alias-label-secondary, #687081); font-size: 12px; }
    .dshSkillsField input { box-sizing: border-box; min-height: 32px; padding: 0 10px; border: 1px solid var(--dsw-alias-border-secondary, rgba(127,127,127,.35)); border-radius: 8px; background: transparent; color: inherit; }
    .dshSkillsTargetGroup { display: flex; gap: 12px; border: 0; padding: 0; margin: 0; }
    ...
    .dshHarnessConversationToolbar span { color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11px; }
    .dshHarnessConversationToolbar button, .dshHarnessChatOptions button { min-height: 30px; padding: 0 10px; border: 1px solid var(--dsh-desktop-divider); border-radius: 8px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); cursor: pointer; white-space: nowrap; }
    .dshHarnessConversationToolbar button:hover, .dshHarnessChatOptions button:hover { border-color: color-mix(in srgb, #7196ff 45%, var(--dsh-desktop-divider)); }
    .dshHarnessChatComposer { display: grid; gap: 16px; width: min(920px, calc(100% - 40px)); box-sizing: border-box; margin: 12px auto 10px; padding: 20px; border: 1px solid color-mix(in srgb, #7196ff 26%, var(--dsh-desktop-divider)); border-radius: 16px; background: var(--dsw-alias-bg-layer-1, #ffffff); }
    .dshHarnessChatComposer > header { display: flex; align-items: flex-start; justify-content: space-between; gap: 18px; }
    .dshHarnessChatComposer h2 { margin: 0; font-size: 20px; }
    .dshHarnessChatComposer header p:last-child { max-width: 660px; margin: 7px 0 0; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 12px; line-height: 1.6; }
    .dshHarnessCurrentModel { flex: 0 0 auto; padding: 6px 9px; border-radius: 999px; color: #a9c6f0; background: color-mix(in srgb, #5877cf 16%, transparent); font-size: 11px; }
    .dshHarnessChatPrompt { display: grid; gap: 7px; color: var(--dsw-alias-label-secondary, #687081); font-size: 12px; }
    .dshHarnessChatPrompt textarea { box-sizing: border-box; width: 100%; min-height: 110px; padding: 11px 12px; border: 1px solid var(--dsh-desktop-divider); border-radius: 10px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); font: inherit; line-height: 1.6; resize: vertical; }
    .dshHarnessChatPrompt textarea:focus-visible, .dshHarnessCapability select:focus-visible { outline: 2px solid var(--dsw-alias-state-business-primary, #7d9cf0); outline-offset: 1px; }
    .dshHarnessCapabilities { display: grid; gap: 9px; }
    .dshHarnessCapabilities h3 { margin: 0; font-size: 13px; }
    .dshHarnessCapabilities > div:first-child p { margin: 4px 0 0; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11px; }
    .dshHarnessCapabilityGrid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 8px; }
    .dshHarnessCapability { display: grid; gap: 10px; min-width: 0; padding: 11px; border: 1px solid var(--dsh-desktop-divider); border-radius: 10px; background: color-mix(in srgb, var(--dsw-alias-bg-layer-2, #f0f2f7) 65%, transparent); }
    .dshHarnessCapability strong { font-size: 12px; }
    .dshHarnessCapability p { margin: 4px 0 0; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11px; line-height: 1.45; }
    .dshHarnessCapability select { box-sizing: border-box; width: 100%; min-height: 30px; padding: 0 7px; border: 1px solid var(--dsh-desktop-divider); border-radius: 7px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); font: inherit; font-size: 11px; }
    .dshHarnessCapabilityEmpty { color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11px; }
    .dshHarnessChatOptions { display: flex; align-items: center; justify-content: space-between; gap: 10px; padding-top: 3px; color: var(--dsw-alias-label-tertiary, #8a92a1); font-size: 11px; }
    .dshHarnessChatActions { display: flex; justify-content: flex-end; }
    .dshHarnessRunStatus { display: grid; gap: 4px; padding: 12px 14px; border: 1px solid var(--dsh-desktop-divider); border-radius: 12px; color: var(--dsw-alias-label-secondary, #687081); background: var(--dsw-alias-bg-layer-2, #f0f2f7); font-size: 12px; }
    .dshHarnessRunStatus strong { color: var(--dsw-alias-label-primary, #202126); font-size: 13px; }
    .dshHarnessAnswer { display: grid; gap: 6px; margin-top: 10px; color: var(--dsw-alias-label-secondary, #687081); font-size: 12px; }
    .dshHarnessAnswer textarea { box-sizing: border-box; width: 100%; min-height: 72px; padding: 9px 10px; border: 1px solid var(--dsh-desktop-divider); border-radius: 8px; color: inherit; background: var(--dsw-alias-bg-layer-2, #f0f2f7); font: inherit; line-height: 1.5; resize: vertical; }
    .dshHarnessPrimary { min-height: 36px; padding: 0 16px; border: 0; border-radius: 9px; color: white; background: #5877cf; font-weight: 600; cursor: pointer; }
    .dshHarnessPrimary:disabled { cursor: default; opacity: .45; }
    /* 官方主题快照始终携带 dsw 变量；这些浅色覆写只兜底仅为主题设计的高亮硬编码色。 */
    body[data-dsh-desktop-theme="light"] .dshPluginMarketCategories button.is-active { color: #3d5aa8; }
    body[data-dsh-desktop-theme="light"] .dshAgentStep[data-state="active"] .dshAgentStepMark { color: #3d5aa8; }
    body[data-dsh-desktop-theme="light"] .dshAgentWorkbenchDiff pre { color: #41507e; }
    @media (max-width: 860px) { .dshHarnessCapabilityGrid { grid-template-columns: 1fr; } }
    @media (max-width: 760px) {
      ...
      .dshHarnessConversationToolbar, .dshHarnessChatComposer { width: calc(100% - 24px); }
      .dshHarnessChatComposer > header, .dshHarnessChatOptions { align-items: flex-start; flex-direction: column; }
    }
