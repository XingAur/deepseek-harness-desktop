...
function parsePlugin(value: unknown): CatalogPlugin {
  ...
  // 纵深防御：安装标识只能指向目录中声明的包名和版本，禁止任何旁路来源。
  if (value.installSpec !== `${value.packageName}@${value.version}`) throw new Error('插件安装标识必须与包名和版本一致')
...
function filterCompatible(catalog: SignedCatalog, target: DesktopTarget, dshVersion: string): SignedCatalog {
  if (valid(dshVersion) === null) throw new Error('Desktop Runtime 提供的 DeepSeek Harness 版本无效')
  return {
    ...catalog,
