...
describe('ExtensionCenter', () => {
  it('shows an explicit review before enabling an extension and uses the scoped bridge action', async () => {
    ...
    expect(screen.getByRole('dialog', { name: '扩展审核详情' })).toBeInTheDocument()
    expect(screen.getByRole('heading', { name: '安全与兼容' })).toBeVisible()
    fireEvent.click(screen.getByRole('button', { name: '返回扩展列表' }))
